...
默认产物是可以直接打开、全文搜索和 Git diff 的 `workspace/baseline/` 目录：
...
```bash
loreloop knowledge search "公积金比例" --package workspace/baseline
```
...
`--format docs` 默认写入可直接阅读的 `workspace/baseline/` 目录；只有需要传输或归档时才使用
`--format package` 生成 `workspace/baseline.zip`。`--format audit` 是另一种单文件逐条信任审计导出，
并不是权威项目文档包。
...
最终报告还会自动写入
`workspace/change/<run-id>/acceptance-report.md`。每次需求或 Bug 都有独立的增量记录，包含
需求/根因分析、实现说明、变更影响、测试选择、执行结果、验收证据和剩余风险。
`workspace/baseline/` 仍是当前项目的全量权威视图；change 报告不会未经治理直接冒充基线事实。
