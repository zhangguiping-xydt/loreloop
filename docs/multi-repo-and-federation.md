...
3. **成员库声明不承载信任语义**:改动 `repos.json` 只能把条目降级(锚点无法解析 → 视为
   drifted),不可能把任何条目升级。因此它可以放在 agent 可写的 `.knowhelm/` 内。

---

## ① 多代码库锚点(workspace)

### 1.1 成员库声明:`.knowhelm/repos.json`
...
新增模块 `src/knowhelm/knowledge/repos.py`:
...
4. 其他任意路径 → **报错退出**,提示先 `knowhelm repo add <path>`。
...
```
knowhelm repo add <path> [--name <name>]     # 缺省 name = 目录名;校验 git 仓库
knowhelm repo list                           # 名称、路径、当前 HEAD(短)、路径是否可达
knowhelm repo remove <name>                  # 仅从 repos.json 移除;条目保留并自然变 drifted
```
...
路径:`$KNOWHELM_REGISTRY` 或缺省 `~/.knowhelm/projects.json`(与 keys 同级,树外)。
测试必须通过 `KNOWHELM_REGISTRY` 指向临时文件,绝不触碰真实 home(与
`KNOWHELM_KEY_DIR` 同一约定)。
...
- 新增模块 `src/knowhelm/federation/registry.py`:load/save/add/remove/list,
  strict 校验,损坏即干净报错。
- CLI:

```
knowhelm project add <path> [--id X] [--name N] [--alias A]... [--tag T]...
knowhelm project list
knowhelm project remove <id>
```

`path` 必须含 `.knowhelm/knowledge.db`,否则拒绝(注册的是信任域,不是任意目录)。
`knowhelm init` 成功后打印一行提示:可用 `knowhelm project add .` 注册以参与联邦检索
(不自动注册——写用户级文件是显式行为)。

### 2.2 只读打开对方信任域

新增模块 `src/knowhelm/federation/reader.py`,核心原则:**联邦 = 以只读方式运行对方
...
```
knowhelm knowledge search <query> [--all | --project <id>] [--limit N=10]
```
...
```
knowhelm knowledge import <project-id> <entry-id-prefix>
```
...
```
knowhelm run <task> [--with-related] [--related-limit N=5]
```
...
| registry 被篡改指向伪项目 | registry 在 `~/.knowhelm/`,与 keys 同一信任边界;honest-workstation 威胁模型(设计文档 §14)原文适用,文档明示 |
