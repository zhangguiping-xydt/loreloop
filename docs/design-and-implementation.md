...
`LORELOOP_KEY_DIR` 可覆盖 key 目录,测试环境也使用该变量隔离真实 home。
...
以 tools 关闭、无 session、空 setting sources/MCP 配置运行；Codex 以 read-only sandbox、
ephemeral、忽略用户配置/规则并在空临时目录运行。实际代码委托使用显式
`acceptEdits`/`workspace-write`,不继承 bypass 模式。
...
在本地；Claude/Codex CLI 仍可能把源码、页面观察和 prompt 发给其配置的外部 provider。
...
LoreLoop 提供两层 host 集成:

1. 仓库 marketplace 中的 Codex plugin 是全局入口。它携带 `$loreloop` skill 和本地
   Runtime 安装器；Runtime 缺失时先向操作者请求明确许可,再从 GitHub Release 下载
   `SHA256SUMS` 与版本化 wheel,校验后安装。插件不执行远端 installer script。
2. `loreloop init` 可在具体项目中安装 Claude Code 和 Codex companion skill,让项目
   即使不通过 marketplace 也携带同一套会话协作边界。
...
Runtime 发行不依赖 PyPI:tag workflow 把版本化 universal wheel、POSIX/PowerShell
...
loreloop harvest <run_id> [--agent claude|codex]
