...
Web 探索和人工浏览器录制都只产生 candidate。完整生命周期是：

```text
exploration/recording
→ .loreloop/web-tests/candidates/*.json
→ operator review
→ chain web_test_approved
→ tests/loreloop/web/*.json
→ Git commit
→ web_test_executed
→ Playwright derivative export
→ acceptance specification and Capsule
```

Scenario v1 是严格 JSON 数据结构，复用 ActionScript 的同源、动作和写入安全边界，并增加标题、
风险、前置条件、标签和确定性断言。candidate 是 0600 私有评审材料，不授予执行权限。`approve`
把精确 JSON 发布到 `tests/loreloop/web/`，再将 canonical digest 和完整场景写入证据链；重放时同时
校验当前文件 digest 与最新链上批准 digest，未批准文件或提交后的篡改都会拒绝执行。
根目录是非 Git 聚合工作区时，单成员仓库可自动成为归属；多个成员仓库必须通过 `--repo` 明确
选择，链上路径使用 `repo:<alias>/tests/loreloop/web/<id>.json`，检测器从该成员仓库的提交态读取。
...
6. 所有 command check 绑定同一个 repository state，且报告生成时项目仍处于该状态。
...
Web coverage 只把当前文件 digest 与链上 `web_test_approved` 完全一致的场景标记为 approved，
历史执行也必须匹配当前场景 digest 才能计入通过状态。
...
旧式 C# 检测还会把字符串 SQL 的读写对象、运行状态消息、后台线程入口、运行时限制、事务提交/
回滚和异常处理器记录为 ImplementationFact。Capsule v5 在其上生成独立的人类语义视图：按运行
单元归纳架构；按已实现能力组织功能；把角色/入口、前置条件、处理行为和数据影响整理为现状规格；
在详细设计中呈现逐行实现证据、跨能力数据协作和数据对象影响矩阵；按功能区域组织 UI 和操作线索；
按显式副作用生成验收候选与覆盖矩阵；并把数据库收敛为核心实体、字段详情和全量索引。任何正式
需求、角色、状态和性能结论仍须显式材料；受控归纳不能制造事实。精确类、
...
日常导出使用 `--format docs` 和目录 `workspace/baseline/`，便于直接打开、Git diff 和检索；只有明确需要
传输或归档时才使用 `--format package` 生成 `workspace/baseline.zip`。两种载体具有相同证明语义，replay
与 search 均直接接受目录或 ZIP，不要求操作者手工解压。

开发过程知识采用独立的增量投影：`loreloop report <run-id>` 除终端输出外，还会原子更新
`workspace/change/<run-id>/acceptance-report.md`。它汇总任务描述、需求/根因分析、实现说明、源码
影响、测试选择与执行、Web 覆盖、验收结果和风险。该报告是证据链的可读投影，不会被源码基线
导出自动当成需求事实；全量权威视图仍由最新源码、测试、显式需求材料及受治理 Web 事实重建。
...
loreloop knowledge export --format docs --output <directory> ...  # 日常可读基线；默认 workspace/baseline/
