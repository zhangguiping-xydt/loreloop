...
- 当前上限为 Capsule 128 MiB、单 Markdown 32 MiB、受管总量 256 MiB。JSON 在构造 Python
  对象图前，以常量额外空间限制 400 万 values、300 万 object members、单容器 10 万项、
  深度 128 和单字符串 8 MiB；ZIP 必须在 `ZipFile` 解析 central directory 前验证最多 16 个
  成员及 64 KiB central-directory 元数据，避免宽度型 JSON/ZIP 先行耗尽内存。
...
其提交必须可从公开 GitHub HTTPS `origin` 的一个显式 remote-tracking ref 到达。Dogfood 参数、公开远端
身份和保留 artifact 是 `passed` manifest 的必需条件；保存命令
日志、摘要、全部 LoreLoop Python 运行时源码哈希与环境；两个独立只读审查者针对同一契约与
