...
运行单元、已实现能力、触发入口、角色边界、前置条件、处理行为、数据读写、事务/异常控制、
UI 功能区和验收候选，并明确区分正式材料与源码反构现状。C# 旧式项目还必须从嵌入 SQL、运行状态
消息、后台线程入口、运行时限制和显式事务调用提取 ImplementationFact；这些事实进入同一
SemanticCore，不能只在 Markdown 中扩写。Capsule SemanticCore 是独立 Agent 视图，保存精确
原子事实、身份、字段和源码绑定。机器闭包由
...
六份核心文档必须承担不同职责，不能仅修改标题后重复同一张接口表：功能清单提供能力总览和逐项
现状说明；需求规格分开呈现正式需求与源码反构现状，并提供角色/入口、前置条件、处理行为和
数据影响映射；系统架构表达运行单元、依赖、配置、宿主和部署边界；详细设计提供实现符号、逐行
原子行为、事务/异常边界、跨能力数据协作和数据对象影响矩阵；用户手册只依据明确 UI/CLI/接口
生成操作线索；验收规格分开呈现正式条款、源码候选、覆盖矩阵和测试证据。接口契约允许保留全部
端点，但必须先提供接口域索引并折叠领域明细。证据不足时必须在对应文档明确声明不能用于完整
需求、操作交付或正式验收，不能用“接口存在”替代业务结论。
