...
文件快照上完成 Capsule replay，再只对人类可见 Markdown 执行有界 BM25；不得要求先导入
SQLite，也不得在验证后重新读取可能已变化的 ZIP。每个可召回事实必须位于一个明确的文档章节，
搜索结果必须返回该 Markdown 文件与章节，不能以 Capsule 中的隐藏事实替代人类文档。
临时索引以章节内段落、列表和表格块为最小上下文单元；不得逐行切碎上下文，也不得把 Mermaid
语法当作知识。长块命中时必须从同一可见块中选择最相关事实作为摘要，不能只截取块首内容。
...
机器闭包由 `.loreloop-export.json` 中的 SemanticCore、确定性 pre-AST 摘要、适用性决策和
Markdown 摘要证明。v3 不再把同一批事实复制进每份完整 AST；重放从 SemanticCore 重建 AST 并
核对摘要。schema v2 的内嵌 AST 包继续兼容重放。
...
同一事实只能有一个规范的人类文档归属。概览负责叙述和导航，`完整知识索引` 负责在折叠领域
明细中列出概览未逐项展示的类、方法、依赖、UI、测试、权限和约束。折叠可以减少视觉噪声，但
不能使事实脱离 Markdown。SemanticCore 负责证明、绑定和确定性重建，不是额外的隐藏知识库。

真实规模门禁必须同时检查：正文不出现原子 `record_id`；概览按领域聚合；完整索引覆盖所有可
召回事实；需求为空时不输出配置伪需求；Capsule 不内嵌重复 AST；至少使用接口名、类/方法名、
数据库字段、测试名和带受控扩展的中文业务词验证召回。任何命中 `.loreloop-export.json` 而不是
人类文档章节的项目知识，都必须判定失败。
...
- 重放必须从 Capsule 内的 SemanticCore 重新构建适用性和完整 DocumentSet。v3 将重建 AST 的
  规范摘要与 Capsule 摘要比较，再逐字节比较 Markdown；v2 还必须验证内嵌 AST 与同一重建结果
  一致。两种格式都不能降级接受不完整的 SemanticCore。
